...
- 不要直接在儲存庫根目錄執行，請使用獨立的研究 workspace 目錄
...
LLM 透過 `TASK.md` 被限制為只回傳 `TARGET_FILE + unified diff`，而且 target 被限定在 `node-prompts/` 內。apply mode 下候選變更必須通過 validation checks；否則會 rollback 並寫入 audit log。`--no-apply` 只建立 context，`--dry-run` 只顯示 diff 而不改檔。
...
- 從 eval harness 與 `run_status.research_process` 讀取元件分數和以產物為依據的過程檢查，不把 readiness 分數視為科學 gate
- 提出有界的 prompt/skill 改進，重新執行 validation，並比較不同 cycle 的過程 blocker
...
AutoLabOS 是一個活躍的 OSS research-engineering 專案。若需要更詳細的說明，請參考 docs 目錄下的文件。
