...
### Adaptive human-intervention contract

Human questions are a shared runtime boundary, not TUI-only presentation state.
The pending request, bounded choices, follow-up turns, selected action, and
interpretation provenance are persisted per run and projected through both TUI
and WebUI.

- `resumeAction` is the declared default, not a decision that silently overrides
  the operator's answer.
- Exact choice identifiers and explicit declared node identifiers may resolve
  without a model call. Natural-language answers may select only a choice
  declared on the pending request, accept the request default, or produce one
  bounded follow-up question.
- An ambiguous answer keeps the request pending. It must not clear the request
  or execute the default action merely because the answer is nonempty.
- Negation is evaluated within decision clauses. A negated declared choice,
  declared default, or verified metric criterion is not promoted by a nearby
  affirmative-looking token. Questions, global refusals, contradictory criteria,
  and action alternatives such as “A or B” fail closed to one bounded follow-up.
- A resolved answer is finalized only after its declared workflow mutation
  succeeds. If the mutation rejects, completed history is not appended and the
  pending request remains available for an explicit retry.
- Model interpretation cannot invent a graph target, waive an evidence or
  readiness blocker, approve an external action, or bypass transition safety.
- TUI and WebUI use the same resolver and supervisor. A fresh or resumed client
  must restore the same pending question and declared choices from run context.
- Completed answers record the resolution source (`exact`, `model`, or
  `guarded_fallback`) and any rationale in human-intervention history.

Answer application has a run-scoped persistence and concurrency contract:

- Before dispatch, the supervisor rechecks the pending request ID, request
  version, and active run state. A stale answer cannot act on a replaced or
  already-consumed question.
- Answer submission and pending-request replacement share a run-scoped SQLite
  lock database under `.autolabos/locks/human-intervention/`. The lock filename
  is derived from a hash of the run ID, and one `BEGIN IMMEDIATE` transaction is
  held across resolve, dispatch, and finalization. SQLite releases the
  transaction if the owner exits, while an in-process async context permits the
  pending-request writer to re-enter the same lock safely. The lock database is
  workspace synchronization state outside the run artifact tree, not a durable
  action receipt.
- `human_intervention/answer_commits/<request-hash>.json` stores one durable
  answer-commit journal per request. `prepared` binds the full request, selected
  action, target, pre-action snapshot, and prospective history before dispatch.
  `dispatching` records that the workflow call may have started.
  `action_applied` is written only after an action-specific durable
  postcondition proves that the declared retry, approval, bound transition, or
  jump was applied. `committed` records the idempotent finalization of history
  and pending-request clearance. A verified unchanged failure may be marked
  `action_failed` and retried without consuming the question. Per-request files
  preserve an older applied action while a downstream question becomes the new
  pending request.
- Reload recovery may finish an `action_applied` journal without dispatching the
  workflow action again, and repeated finalization does not duplicate history.
  A journal left at `dispatching` has an unknown action outcome and therefore
  fails closed with recovery required. It is never blindly re-executed; an
  operator must reconcile the run and journal before further answers are
  accepted. This is the explicit crash window where automatic exactly-once
  recovery cannot be proven.

This adaptive dialogue boundary does not make the governed workflow itself
model-controlled. State transitions, evidence gates, approval checks, claim
ceilings, and the fixed node allowlist remain deterministic.

### Adaptive guided-brief interview contract

The human TUI path for `/new` and the WebUI guided new-run path use a shared
field-coverage state machine rather than treating the Research Brief as a fixed
questionnaire. One operator answer may cover several declared brief fields, so
the next prompt is selected from the remaining required coverage instead of a
hard-coded step number.

- Explicit `Field: value` segments resolve locally. Free-form narrative may use
  a model interpreter, but every stored extraction must be an exact substring of
  the operator answer and must target a declared brief field.
- An answer that leaves the current field uncertain keeps that field pending and
  produces one focused follow-up. Interpreter failure falls back to accepting
  only the current field; it does not fabricate cross-field coverage.
- Every guarded fallback carries one bounded reason code, such as provider
  authentication, request rejection, rate limiting, timeout, transport, empty
  output, invalid JSON, or invalid schema. Raw provider errors and model output
  are not projected to the TUI/WebUI or stored as the diagnostic.
- Required Research Brief fields, discovery-mode scope fields, completeness
  validation, and downstream research gates remain deterministic.
- Optional fields are offered only after required coverage is complete and may
  be declined as a group.
- Explicit file-driven guided-brief automation retains the historical positional
  22/25-step path for reproducible fixtures. It does not invoke adaptive model
  interpretation.
- The Web server owns each guided draft and projects only its prompt, coverage,
  provenance, bounded fallback reason, and generated brief to React. The client
  does not calculate field coverage or declare an interview complete.
- ChatGPT-backed Codex requests include the account identifier from the local
  OAuth credential record when it is available. If the configured model is
  rejected, the interview reports `provider_request_rejected` and keeps the
  current-field fallback; it does not silently switch models.
- Guided Web drafts are bounded, process-local, and explicitly non-durable. A
  server restart expires an unfinished interview; completing it yields a normal
  governed brief that still passes through the existing Web start gate.
- Pasting an already complete governed brief remains an explicit advanced Web
  path. It does not restore the removed Topic / Constraints / Objective shortcut
  form or bypass brief completeness validation.

### Provider compatibility diagnostics contract

The default TUI `/doctor` command and `GET /api/doctor` are local diagnostics.
They inspect environment and workspace state without sending a live Codex chat
or generation request. Existing non-generation diagnostics may still query a
configured local runtime for health or model availability. A live Codex chat
request is permitted only through the exact TUI command `/doctor --live-provider`
or the explicit live-provider action in the Web Doctor surface.

The opt-in path has a deliberately narrow contract:

- It sends exactly one fixed, non-user Codex chat request and never retries.
- It uses the configured Codex `chat_model`, falling back to the configured
  `model` only when `chat_model` is absent. It does not switch models after a
  rejection.
- It may consume provider quota and therefore must not run on page load,
  refresh, or any other implicit diagnostic path.
- It projects only a bounded status: `compatible`, `fixture_active`,
  `auth_unavailable`, `request_rejected`, `rate_limited`, `timeout`,
  `transport_error`, `empty_response`, or `provider_error`.
- Provider output, response bodies, and credentials are neither projected into
  the diagnostic result nor persisted by the probe.

A `compatible` result establishes one-shot chat compatibility for the configured
route only. It does not establish research execution readiness, PDF generation,
long-running reliability, or paper readiness.

### General Codex OAuth completion failure boundary

Normal Codex OAuth completions continue to return provider text, usage, and
response identity on success. Provider, transport, local input-preparation, and
completion progress-observer failure diagnostics cross the integration boundary
only as a typed, allowlisted error. The stable categories are authentication
unavailable, request rejected, local input unavailable, quota exhausted,
temporary rate limiting, transport failure, provider unavailable, stream
terminated, incomplete response, empty response, unspecified provider failure,
and local progress-observer failure.

Success requires an allowlisted output terminal. A text delta followed by EOF,
or a reasoning/content-part completion without an output terminal, is an
incomplete response even when partial text exists. Response identity is committed
only after terminal validation and all completion observers succeed.

Only output-text evidence participates in the final answer. Reasoning deltas and
reasoning items are ignored. A structured `response.completed` output is
authoritative; otherwise the latest valid structured output-terminal candidate
precedes an exact `response.output_text.delta` fallback, and that fallback is
eligible only after an allowlisted output terminal. Pre-terminal response
snapshots and stale partial text cannot establish success or override a later
terminal output. The opt-in compatibility probe applies the same explicit
terminal and output-only boundary before reporting `compatible`.

HTTP bodies, streamed provider error messages, incomplete-response reasons,
network exception details, and local input paths must not be copied into the typed
error message, cause, TUI/Web projection, event log, checkpoint, reflexion record,
or run artifact. A
bounded HTTP 429 parser may parse `type` and `code` metadata but retains only
whether either value matches an allowlisted quota identifier; all raw metadata is
then discarded. External operator cancellation remains an `AbortError`,
while a provider-authored termination message is a provider stream failure and
must not be projected as user cancellation. Callers use the typed retry
disposition for backoff or caller recovery instead of parsing provider prose.
...
- Human-answer application uses the request-scoped
  `human_intervention/answer_commits/<request-hash>.json` durable journals
  described above. Cross-process serialization uses the hashed workspace-level
  SQLite lock database under `.autolabos/locks/human-intervention/`; neither the
  lock database nor an in-flight journal is completed research evidence.
