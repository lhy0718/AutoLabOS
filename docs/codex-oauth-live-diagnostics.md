...
The failure phenotypes below are historical validation evidence. Current Codex
OAuth failures handled by the shared completion boundary are projected with typed
safe categories such as `provider_unavailable`, `stream_terminated`, or
`quota_exhausted`; provider response bodies, streamed error messages, network
causes, and request identifiers are not retained as the operator-facing failure
diagnostic. Successful model output remains governed by the normal run-artifact
contract.
