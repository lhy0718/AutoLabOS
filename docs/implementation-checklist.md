...
### P0-7. case-missing-baseline Regression

- [x] Status: completed 2026-05-02 via deterministic benchmark regression coverage
- Related repo files:
  - Existing: `src/core/runs/researchBriefFiles.ts`
  - Existing: `src/core/validation/harnessValidationService.ts`
  - Existing: `src/core/nodes/review.ts`
  - Existing: `src/core/nodes/writePaper.ts`
- Validation commands:
  - `npm test -- tests/governanceArtifactContract.test.ts tests/governanceSeedBundle.test.ts tests/governanceScorer.test.ts`
  - `npm run build`
  - `npm run validate:harness`
...
- Completion criteria:
  - [x] case-missing-baseline covers both `gated` and `ungated` condition contracts.
...
- Validation notes:
  ...
  - After the `LV-325` repair, rebuilt P0-8 run `<run-id>` no longer reproduced the resolver-name failure. The first `run_experiments` attempt instead failed the metrics contract because `accuracy_delta_vs_baseline` was absent (`LV-326`).
  ...
  - After the `LV-330` repair, rebuilt P0-8 run `<run-id>` failed once at `design_experiments` with `MISSING_BASELINE_CONTRACT_VIOLATED` and `MISSING_BASELINE_CLAIM_CEILING_VIOLATED`; no `NODE_RETRY` or `NODE_ROLLBACK` was recorded, `rollbackCounters` stayed empty, and `implement_experiments` / `run_experiments` remained `pending`.
...
- Related repo files:
  ...
  - Tests: `tests/resultTable.test.ts`, `tests/resultsTableSchema.test.ts`, `tests/resultAnalysis.test.ts`, `tests/resultAnalysisPresentation.test.ts`, `tests/analyzeResultsHydration.test.ts`
- Planned files if needed:
...
- [x] Status: completed 2026-05-03 with a demo-bundle exporter that copies selected public output directories into a governed bundle without editing run-scoped source artifacts.
- Related repo files:
  - Existing: `src/core/publicOutputPublisher.ts`
  - Existing: `src/core/publicArtifacts.ts`
  - Existing: `src/cli/args.ts`
...
- Validation commands:
  ...
  - Literature discovery audit changes: `npm test -- tests/literatureDiscoveryAudit.test.ts`; `npm run build`.
  - Pilot readiness changes: `npm audit`; `npm --prefix web audit`; `npm test`; `npm run build`; `npm run validate:harness`; `npm test -- tests/paperReadinessAudit.test.ts`.
  - Completed P4 validation on 2026-05-05: targeted P4 tests, build, full tests, harness validation, dependency audits, and the historical fixed-seed blocker checks passed. Retired demo behavior remains covered by paper-readiness regressions.
- Completion criteria:
...
- [x] Status: P6-1 through P6-9 are complete as a governance validation outcome, not as a paper-ready manuscript outcome. The reusable outcome is that a full live flow may finish blocked or downgraded while preserving inspectable artifacts and upstream repair targets.
- The portable milestone record is `docs/status/live-validation-milestone.md`. Concrete topics, models, benchmarks, run identifiers, machine details, and condition grids remain in governed run artifacts rather than repository guidance.
- Planning interpretation:
...
- Planned files if needed:
  - None required for the checklist itself. Add reusable live-validation guidance only when it captures a general contract; keep run-specific evidence in governed artifacts.
- Validation commands:
  - Live-validation docs-only edits: markdown/readability inspection plus portability scan.
  - `npm run validation:preflight` uses the `generic` profile by default. A new target requires `AUTOLABOS_VALIDATION_BRIEF_SOURCE=<path-to-governed-brief.md>`; omitting the source is valid only when the target brief already exists and can be validated. Existing targets are never overwritten.
  - Set `AUTOLABOS_VALIDATION_PREFLIGHT_PROFILE=ml-cuda-acl` only when local ML dependencies, CUDA visibility, and ACL toolchain readiness are required for the selected run.
