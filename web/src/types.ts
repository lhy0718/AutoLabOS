...
export interface RunResearchFunnelCandidateProjection {
  ...
  topic_memory_disposition?: "clear" | "blocked" | "requires_reentry_adjudication" | "reentry_allowed";
...
export interface WebSessionState {
  ...
  humanIntervention?: HumanInterventionView;
  logs: string[];
  canCancel: boolean;
  activeRunInsight?: RunInsightCard;
}

export interface HumanInterventionViewChoice {
  id: string;
  label: string;
  description?: string;
}
...
export interface HumanInterventionView {
  ...
  question: string;
  context: string[];
  inputMode: "free_text" | "single_choice";
  choices: HumanInterventionViewChoice[];
...
export type WebRunCreationResponse =
  | {
      created: false;
  ...
  | {
      created: true;
      startOutcome: "not_requested" | "scheduled" | "deferred";
      run: RunRecord;
      session: WebSessionState;
      runs: RunRecord[];
      briefStartGate: ResearchBriefStartGate;
...
export interface WebGuidedBriefInterview {
  id: string;
  ...
  lastResolutionSource?: "labeled_input" | "model" | "guarded_fallback" | "operator_control";
  lastFallbackReason?: AdaptiveGuidedBriefFallbackReason;
  generatedBrief?: string;
}

export interface WebGuidedBriefInterviewResponse {
  interview: WebGuidedBriefInterview;
}
