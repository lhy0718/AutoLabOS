...
2. On first use inside the AutoLabOS repository, inspect the plugin contract with `npm run plugin:contract` and run `npm run plugin:dogfood`. Treat a passing dogfood report as plugin-contract coherence only, not as research completion.
