...
const allowedIntents = new Set(["new", "audit", "review", "improve", "pack"]);

if (args.includes("--help") || args.includes("-h") || args.length === 0) {
  process.stdout.write([
    "Usage: run-research-intent.mjs <new|audit|review|improve|pack> [autolabos research options]",
...
if (args[0] === "--check") {
  ...
  const contractCompatible = Boolean(
    available
    && helpResult?.status === 0
    && ["new", "audit", "review", "improve", "pack"].every((intent) => helpText.includes(intent))
  );
