...
import type {
  CodexOAuthCompatibilityProbeOptions,
  CodexOAuthCompatibilityProbeResult
} from "./oauthResponsesTextClient.js";
...
export class CodexNativeClient {
  ...
  async probeChatCompatibility(
    opts: CodexOAuthCompatibilityProbeOptions
  ): Promise<CodexOAuthCompatibilityProbeResult> {
    if (hasFakeCodexResponseConfigured()) {
      return { status: "fixture_active" };
    }
    return this.oauthTextClient.probeCompatibility(opts);
  }
...
function hasFakeCodexResponseConfigured(): boolean {
  return Boolean(
    typeof process.env.AUTOLABOS_FAKE_CODEX_RESPONSE_SEQUENCE === "string"
    || typeof process.env.AUTOLABOS_FAKE_CODEX_RESPONSE === "string"
  );
}
