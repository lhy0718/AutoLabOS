...
import {
  ...
  WebGuidedBriefInterviewLimitError,
...
export const WEB_TRUSTED_ORIGIN_ENV = "AUTOLABOS_WEB_TRUSTED_ORIGIN";
const WEB_AUTH_MIN_SECRET_BYTES = 16;
const WEB_AUTH_MAX_HEADER_BYTES = 8 * 1024;
const GUIDED_BRIEF_LANGUAGES = new Set(
  listGuidedBriefInterviewLanguages().map((option) => option.value)
);
const GUIDED_BRIEF_RESEARCH_MODES = new Set<GuidedBriefResearchMode>([
  "hypothesis_test",
  "topic_discovery"
]);
...
interface DoctorResponseBuildOptions {
  liveProviderProbe?: {
    model: string;
  };
}
...
export function createAutoLabOSWebRequestListenerForTesting(opts: {
  cwd: string;
  runtime?: AutoLabOSRuntime;
  trustedOrigin?: string;
}): WebRequestHandler {
  const controller = new AutoLabOSWebController(
    {
      cwd: opts.cwd,
  ...
  return controller.createAuthenticatedRequestListener();
}
...
class AutoLabOSWebController {
  ...
  private readonly trustedRequestOrigin?: string;
  ...
  private guidedBriefInterviews?: WebGuidedBriefInterviewManager;
  private sessionUnsubscribe?: () => void;
  private eventUnsubscribe?: () => void;
  private doctorProviderProbeInFlight?: Promise<DoctorResponse>;
  private readonly sseClients = new Set<ServerResponse>();

  constructor(opts?: WebServerOptions, initialRuntime?: AutoLabOSRuntime) {
    this.cwd = opts?.cwd || process.cwd();
    this.host = opts?.host?.trim() || "127.0.0.1";
    this.port = opts?.port || 4317;
    this.authentication = resolveWebAuthentication(this.host);
    this.trustedRequestOrigin = resolveTrustedWebOrigin(
      opts?.trustedOrigin !== undefined
        ? opts.trustedOrigin
        : process.env[WEB_TRUSTED_ORIGIN_ENV]
    );
  ...
  createRequestListener(): WebRequestHandler {
    return (req, res) => this.handleRequest(req, res);
  }

  createAuthenticatedRequestListener(): (req: IncomingMessage, res: ServerResponse) => void {
    return createAuthenticatedWebRequestListener(
      this.authentication,
  ...
  async start(): Promise<void> {
    ...
    const server = http.createServer(this.createAuthenticatedRequestListener());
  ...
  private async attachRuntime(runtime: AutoLabOSRuntime): Promise<void> {
    ...
    this.doctorProviderProbeInFlight = undefined;
    ...
    this.guidedBriefInterviews = new WebGuidedBriefInterviewManager({
      resolveAnswer: (input) => session.resolveGuidedBriefInterviewAnswer(input)
    });
  ...
  private async handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (pathname === "/api/doctor" && method === "GET") {
        return jsonResponse(res, 200, await this.buildDoctorResponse());
      }

      if (pathname === "/api/doctor/provider-probe" && method === "POST") {
        if (!hasMatchingRequestOrigin(req, this.trustedRequestOrigin)) {
          return jsonResponse(res, 403, { error: "Provider compatibility check request was rejected." });
        }
        ...
        try {
          ...
          return jsonResponse(res, 400, { error: "A valid provider compatibility confirmation is required." });
        }
        if (!hasExactProviderProbeConfirmation(body)) {
          return jsonResponse(res, 400, { error: "A valid provider compatibility confirmation is required." });
        }
        if (!this.runtime || !isCodexLlmMode(this.runtime.config.providers.llm_mode)) {
          return jsonResponse(res, 409, {
            error: "The configured runtime does not support this provider compatibility check."
          });
        }
        try {
          return jsonResponse(res, 200, await this.runDoctorProviderProbe());
        } catch {
          return jsonResponse(res, 500, { error: "Provider compatibility check could not be completed." });
        }
      }
      ...
      if (pathname === "/api/guided-brief-interviews" && method === "POST") {
        const interviews = this.requireGuidedBriefInterviews(res);
        if (!interviews) {
          return;
        }
        const body = (await readJsonBody(req)) as JsonBody;
        const language = asTrimmedString(body.language) as GuidedBriefInterviewLanguage | undefined;
        const researchMode = asTrimmedString(body.researchMode) as GuidedBriefResearchMode | undefined;
        if (!language || !GUIDED_BRIEF_LANGUAGES.has(language)) {
          return jsonResponse(res, 400, { error: "language must be one of the supported guided brief languages." });
        }
        if (!researchMode || !GUIDED_BRIEF_RESEARCH_MODES.has(researchMode)) {
          return jsonResponse(res, 400, { error: "researchMode must be hypothesis_test or topic_discovery." });
        }
        const templateDefault = await fs.stat(path.join(this.cwd, "template.tex"))
          .then((stat) => stat.isFile() ? "template.tex" : "")
          .catch(() => "");
        return jsonResponse(res, 201, {
          interview: interviews.start({ language, researchMode, templateDefault })
        });
      }

      const guidedBriefAnswerMatch = pathname.match(/^\/api\/guided-brief-interviews\/([^/]+)\/answers$/u);
      if (guidedBriefAnswerMatch && method === "POST") {
        const interviews = this.requireGuidedBriefInterviews(res);
        if (!interviews) {
          return;
        }
        ...
        if (typeof body.answer !== "string") {
          return jsonResponse(res, 400, { error: "answer must be a string." });
        }
        const interview = await interviews.answer({
          id: decodeURIComponent(guidedBriefAnswerMatch[1]),
          answer: body.answer
        });
        return jsonResponse(res, 200, { interview });
      }

      const guidedBriefMatch = pathname.match(/^\/api\/guided-brief-interviews\/([^/]+)$/u);
      if (guidedBriefMatch && method === "GET") {
        const interviews = this.requireGuidedBriefInterviews(res);
        if (!interviews) {
          return;
        }
        return jsonResponse(res, 200, {
          interview: interviews.get(decodeURIComponent(guidedBriefMatch[1]))
        });
      }
      if (guidedBriefMatch && method === "DELETE") {
        const interviews = this.requireGuidedBriefInterviews(res);
        if (!interviews) {
          return;
        }
        return jsonResponse(res, 200, {
          cancelled: interviews.cancel(decodeURIComponent(guidedBriefMatch[1]))
        });
      }
      ...
      if (pathname === "/api/runs" && method === "POST") {
        ...
        return jsonResponse(res, 200, {
          created: true,
          startOutcome: autoStart ? "scheduled" : "not_requested",
          run,
          session: session.snapshot(),
          runs: session.runs(),
          briefStartGate
      ...
      const status = error instanceof ResearchRunInputError || error instanceof OllamaModelConfigurationError
        ? 400
        : error instanceof WebGuidedBriefInterviewNotFoundError
          ? 404
          : error instanceof WebGuidedBriefInterviewBusyError
            ? 409
            : error instanceof WebGuidedBriefInterviewLimitError
              ? 429
              : 500;
  ...
  private async buildDoctorResponse(opts?: DoctorResponseBuildOptions): Promise<DoctorResponse> {
    if (!this.runtime) {
    ...
    const report = await runDoctorReport(this.runtime.codex, {
      ...
      ...(opts?.liveProviderProbe ? { liveProviderProbe: opts.liveProviderProbe } : {}),
  ...
  private runDoctorProviderProbe(): Promise<DoctorResponse> {
    if (this.doctorProviderProbeInFlight) {
      return this.doctorProviderProbeInFlight;
    }
    const runtime = this.runtime;
    if (!runtime || !isCodexLlmMode(runtime.config.providers.llm_mode)) {
      return Promise.reject(new Error("Provider compatibility check is unavailable."));
    }
    const model = runtime.config.providers.codex.chat_model || runtime.config.providers.codex.model;
    let pending!: Promise<DoctorResponse>;
    pending = this.buildDoctorResponse({ liveProviderProbe: { model } }).finally(() => {
      if (this.doctorProviderProbeInFlight === pending) {
        this.doctorProviderProbeInFlight = undefined;
      }
    });
    this.doctorProviderProbeInFlight = pending;
    return pending;
  }
  ...
  private requireGuidedBriefInterviews(
    res: ServerResponse
  ): WebGuidedBriefInterviewManager | undefined {
    if (!this.guidedBriefInterviews) {
      jsonResponse(res, 409, { error: "AutoLabOS is not configured yet." });
      return undefined;
    }
    return this.guidedBriefInterviews;
  }
...
function resolveTrustedWebOrigin(value: string | undefined): string | undefined {
  ...
  try {
    ...
    throw new Error(
      `${WEB_TRUSTED_ORIGIN_ENV} must be an absolute HTTP(S) origin without credentials, path, query, or fragment.`
    );
  }
}

function hasMatchingRequestOrigin(
  req: IncomingMessage,
  trustedRequestOrigin?: string
): boolean {
  ...
  try {
    ...
    const directRequestOrigin = new URL(req.url || "/", `http://${requestHost}`).origin;
    return parsedOrigin.origin === directRequestOrigin
      || parsedOrigin.origin === trustedRequestOrigin;
  } catch {
    return false;
  }
}
...
function hasExactProviderProbeConfirmation(value: unknown): value is { confirm: true } {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  const body = value as Record<string, unknown>;
  return body.confirm === true && Object.keys(body).length === 1;
}
