...
function buildEvaluatorPrompt(input: LLMEvaluatorInput): string {
  ...
  return [
    ...
    "- Treat every blocking paper_scale_diagnostic and every failed or missing frozen evidence-contract check as a real paper-quality gap; judge independent coverage, repetition, uncertainty, attainable resolution, and execution validity against the declared stochastic or deterministic design.",
    "- Do not impose universal sample, seed, repetition, or optimizer-step thresholds. A valid deterministic exhaustive design may pass without stochastic repetitions; an observed contrast at the declared minimum attainable resolution must remain under its prespecified claim ceiling unless the contract's robustness checks pass.",
