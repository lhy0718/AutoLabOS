  ...
  return {
    ...
    async execute({ run, abortSignal }) {
      ...
      if (preFailureMemoryScriptPath && (await fileExists(preFailureMemoryScriptPath))) {
        ...
        const plannedRunSpecMappingPreRepair =
          await repairPythonPlannedRunSpecMappingSurface(preFailureMemoryScriptPath);
        if (plannedRunSpecMappingPreRepair.repaired) {
          preFailureMemoryRepairApplied = true;
          preFailureMemoryRepairMessages.push(
            plannedRunSpecMappingPreRepair.message?.replace(
              "before handoff.",
              "before run_experiments retry gate."
...
async function repairPythonRuntimeCompatibilityBeforeRun(input: {
  ...
  if (entrypointLookupHelperAliasRepair.repaired) {
    repaired = true;
    messages.push(
      entrypointLookupHelperAliasRepair.message?.replace(
        "before handoff.",
        "before run_experiments execution."
  ...
  const evaluationChoiceBatchScoringRepair =
    await repairPythonEvaluationChoiceBatchScoringSurface(scriptPath);
  if (evaluationChoiceBatchScoringRepair.repaired) {
    repaired = true;
    messages.push(
      evaluationChoiceBatchScoringRepair.message?.replace(
        "before handoff.",
        "before run_experiments execution."
