...
import { appendDelegationTrace, prepareDelegationContractForRun } from "../governance/delegationContract.js";
...
export class TerminalApp {
  ...
  private async runGuidedBriefInterview(workspaceRoot: string): Promise<string> {
    if (await this.getGuidedBriefAutomation()) {
      return this.runPositionalGuidedBriefInterview(workspaceRoot);
    }
    ...
    let state = createAdaptiveGuidedBriefState({
      language: interviewLanguage,
      researchMode
    });
    this.pushLog(`Adaptive guided brief mode: ${researchMode}`);

    for (let turn = 1; turn <= 40; turn += 1) {
      const prompt = getNextAdaptiveGuidedBriefPrompt(state, copy, templateDefault);
      if (prompt.kind === "complete") {
        return buildGuidedResearchBriefMarkdown(buildAdaptiveGuidedBriefAnswers(state));
      }

      const coverage = summarizeAdaptiveGuidedBriefCoverage(state);
      this.pushLog(
        `Guided brief coverage ${coverage.answered}/${coverage.required}; next=${prompt.kind === "field" ? prompt.field : "optional_preferences"}.`
      );
      const answer = prompt.required
        ? await this.askRequiredGuidedBriefQuestion(prompt.question, prompt.defaultValue, copy.requiredAnswerMessage)
        : await this.askGuidedBriefQuestion(prompt.question, prompt.defaultValue);
      ...
      if (resolution.acceptedFields.length > 0) {
        this.pushLog(
          `Guided brief accepted fields via ${resolution.source}: ${resolution.acceptedFields.join(", ")}.`
        );
      }
      if (resolution.fallbackReason) {
        this.pushLog(`Guided brief interpretation fallback: ${resolution.fallbackReason}.`);
      }
      if (resolution.status === "followup_required") {
        this.pushLog("The previous answer needs one focused clarification before the interview can advance.");
      }
  ...
  private async runPositionalGuidedBriefInterview(workspaceRoot: string): Promise<string> {
    const interviewLanguage = await this.selectGuidedBriefInterviewLanguage();
    const languageCopy = getGuidedBriefInterviewCopy(interviewLanguage);
    for (const line of languageCopy.introLines) {
      this.pushLog(line);
