...
import {
  buildDoctorHighlightLines,
  getDoctorCheckStatus,
...
import { getGuidedBriefInterviewCopy } from "../core/runs/guidedBriefInterview.js";
...
export class InteractionSession {
  ...
  constructor(deps: InteractionSessionDeps) {
    ...
    this.interactiveSupervisor = new InteractiveRunSupervisor(
      this.workspaceRoot,
      deps.runStore,
      deps.orchestrator
    );
  ...
  snapshot(): WebSessionState {
    return {
      ...
      pendingPlan: this.pendingNaturalCommand
        ...
        : undefined,
      humanIntervention: this.pendingHumanIntervention
        ? {
            runId: this.pendingHumanIntervention.runId,
            requestId: this.pendingHumanIntervention.request.id,
            sourceNode: this.pendingHumanIntervention.request.sourceNode,
            kind: this.pendingHumanIntervention.request.kind,
            title: this.pendingHumanIntervention.request.title,
            question: this.pendingHumanIntervention.request.question,
            context: [...this.pendingHumanIntervention.request.context],
            inputMode: this.pendingHumanIntervention.request.inputMode,
            choices: (this.pendingHumanIntervention.request.choices || []).map((choice) => ({
              id: choice.id,
              label: choice.label,
              description: choice.description
            })),
            conversationTurnCount: this.pendingHumanIntervention.request.conversation?.length || 0
          }
        : undefined,
      logs: [...this.logs],
      canCancel: this.busy,
  ...
  async resolveGuidedBriefInterviewAnswer(input: {
    state: AdaptiveGuidedBriefState;
    answer: string;
    templateDefault?: string;
    abortSignal?: AbortSignal;
  }): Promise<AdaptiveGuidedBriefResolution> {
    return resolveAdaptiveGuidedBriefAnswer({
      ...input,
  ...
  async submitInput(text: string): Promise<WebSessionState> {
    ...
    if (this.pendingHumanIntervention && !isSlashPrefixed(normalized)) {
      await this.handlePendingHumanInterventionAnswer(normalized);
      return this.snapshot();
    }
    ...
    await this.runBusyAction(
      async (abortSignal) => {
        ...
        if (result.status === "followup_required") {
          this.pushLog(`Follow-up required: ${result.message}`);
  ...
  private async continueSupervisedRun(runId: string, abortSignal?: AbortSignal): Promise<RunRecord> {
    ...
    if (outcome.status === "paused") {
      this.pushLog(`Run paused: ${oneLine(outcome.reason)}`);
      return outcome.run;
    }
    if (outcome.status === "completed") {
      this.pushLog(`Research finished: ${oneLine(outcome.summary)}`);
      return outcome.run;
    }
    this.pushLog(`Research stopped: ${oneLine(outcome.summary)}`);
    return outcome.run;
  }
  ...
  private announceHumanIntervention(request: HumanInterventionRequest): void {
    ...
    this.pushLog("Answer in the WebUI composer. Declared choices remain bounded by the workflow contract.");
  }
  ...
  private async executeParsedSlash(
    ...
    switch (command) {
      ...
      case "doctor":
        return this.handleDoctor(args, abortSignal);
      case "runs":
  ...
  private printHelp(): void {
    ...
    this.pushLog("/doctor [--live-provider] | /approve | /retry");
  ...
  private async handleDoctor(args: string[], abortSignal?: AbortSignal): Promise<SlashExecutionResult> {
    const parsed = parseDoctorCommandArgs(args);
    if (parsed.error) {
      this.pushLog(parsed.error);
      return { ok: false, reason: "invalid doctor options" };
    }

    const llmMode = this.config.providers.llm_mode;
    if (parsed.liveProvider && llmMode !== "codex" && llmMode !== "codex_chatgpt_only") {
      const reason = "Live provider compatibility check requires a configured Codex chat provider.";
      this.pushLog(reason);
    ...
    const report = await runDoctorReport(this.codex, {
      ...
      liveProviderProbe: parsed.liveProvider
        ? {
