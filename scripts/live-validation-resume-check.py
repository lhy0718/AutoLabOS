...
def main() -> int:
    ...
    expected_run_id = os.environ.get("AUTOLABOS_VALIDATION_RUN_ID", "").strip()
    ...
    if expected_run_id:
        ...
        if record.get("status") == "running" and node_status(record, node) == "running":
            output_path = output_dir / "live-validation-resume-check-output.txt"
            output_path.write_text(
                f"SKIP_TUI_ACTIVE_RUNNING run={expected_run_id} node={node}\n",
                encoding="utf-8"
            )
            print(
                "PASS: Validation run is actively running; skipped opening a TUI resume-check so the live node is not interrupted."
            )
            return 0
    ...
    if "[ATTN] readiness:" in buffer_text or "[FAIL] harness-validation:" in buffer_text:
        ...
        return 1
    print(f"PASS: Validation resumed session and /doctor completed; output={output_path}")
    return 0
