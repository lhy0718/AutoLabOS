...
function runResearch(workspace, args) {
  const result = spawnSync(process.execPath, [cliPath, "research", ...args], {
    cwd: workspace,
    encoding: "utf8",
    env: process.env
  });
...
function main() {
  if (process.argv.slice(2).some((arg) => arg === "--help" || arg === "-h")) {
    process.stdout.write("Usage: npm run validate:research-governance\nRequires a current npm run build output.\n");
    return;
  }
  ...
  const workspace = fs.mkdtempSync(path.join(validationRoot, "autolabos-research-governance-e2e-"));
  const checks = [];
  let processCount = 0;
  ...
  try {
    const run = (args) => {
      processCount += 1;
      return runResearch(workspace, args);
    };
    ...
    const report = {
      ...
      processCount,
      checks,
      validationCommand: "npm run validate:research-governance"
    };
