...
describe("research validation run", () => {
  ...
  it("keeps the repository paper-scale profile structurally executable", async () => {
    const sourceProfile = await fs.readFile(path.resolve(
      process.cwd(),
      "docs/research/final-validation-profile.json"
    ));
    await fs.mkdir(path.join(workspace, "docs", "research"), { recursive: true });
    await fs.mkdir(path.join(workspace, "papers", "promotion-governance"), { recursive: true });
    await fs.writeFile(path.join(
      workspace,
      "docs",
      "research",
      "final-validation-profile.json"
    ), sourceProfile);

    const result = await runResearchValidation({
      cwd: workspace,
      profilePath: "docs/research/final-validation-profile.json",
      outDir: "outputs/profile-contract"
    }, {
      executeCommand: async (input) => {
        const reportIndex = input.args.indexOf("--report");
        if (reportIndex >= 0) {
          const reportPath = input.args[reportIndex + 1];
          await fs.writeFile(reportPath, Buffer.from("{\"verdict\":\"pass\"}\n"));
        }
        const outDirArg = input.args.find((arg) => arg.startsWith("-outdir="));
        if (outDirArg) {
          const paperOutDir = outDirArg.slice("-outdir=".length);
          await fs.mkdir(paperOutDir, { recursive: true });
          await fs.writeFile(path.join(paperOutDir, "manuscript.pdf"), Buffer.from("pdf"));
        }
        if (input.command === "pdftoppm") {
          const outputPrefix = input.args.at(-1);
          if (!outputPrefix) throw new Error("Missing PDF render output prefix.");
          await fs.writeFile(`${outputPrefix}-1.png`, Buffer.from("png"));
        }
        return commandResult(0, `${input.command} passed`);
      },
      inspectRepository: cleanRepositoryInspector()
    });
    ...
    expect(result.report).toMatchObject({
      passed: true,
      profile: { id: "paper-scale-release-validation" },
      summary: {
    ...
    expect(result.report.steps.map((stepResult) => stepResult.id)).toEqual([
      "build",
      "full_tests",
      "web_tests",
      "harness",
