...
describe("TerminalApp pending natural plan execution", () => {
    ...
    try {
      ...
      app.askWithinTui = vi
        ...
        .mockResolvedValueOnce([
          ...
          "Scientific object: automated research evaluation",
          "Empirical problems: decision stability under finite evidence and uncertainty calibration under dependent tasks",
          "Prior work probes: whether adaptive evaluation already subsumes the proposed scope",
          "Primary metric: one judge-independent primary score per candidate",
          "Meaningful improvement: a declared practical effect boundary",
          "Constraints: single workstation with licensed public data and fixed budgets",
          "Baseline: a strong declared comparator for each candidate",
          "Dataset: licensed public data with a frozen split and independent units"
        ].join("; "))
      ...
      expect(app.logs).toContain("Adaptive guided brief mode: topic_discovery");
      expect(app.logs).toContain("Guided brief coverage 0/18; next=topic.");
      expect(app.logs).toContain("Guided brief coverage 9/18; next=researchQuestion.");
      expect(app.logs.some((line: string) => line.includes("Guided brief accepted fields via labeled_input"))).toBe(true);
      expect(app.logs.some((line: string) => line.includes("Guided brief step"))).toBe(false);
  ...
  it("asks guided /new interview questions in the selected language", async () => {
    ...
    try {
      ...
      app.askWithinTui = vi
        .fn()
        .mockResolvedValueOnce([
          "주제: 후보 조건과 기준 조건을 제한된 절차로 비교",
          "주요 평가 지표: primary_score",
          "의미 있는 개선 기준: primary_score +0.05",
          "제약 조건: 공개 데이터와 로컬 예산만 사용",
          "베이스라인: reference_condition",
          "데이터셋: 공개 데이터셋의 선언된 조건 비교"
        ].join("; "))
        ...
        .mockResolvedValueOnce("아니요")
        .mockResolvedValueOnce("n");

      await app.handleNewRun();

      expect(app.askWithinTui).toHaveBeenNthCalledWith(
        1,
        "주제 — 이미 정한 지표, 개선 기준, 비교 대상, 데이터, 제약도 함께 설명하셔도 됩니다. 답한 항목은 다시 묻지 않습니다.",
        ""
      );
      expect(app.askWithinTui).toHaveBeenNthCalledWith(2, "연구 질문", "");
      expect(app.logs).toContain("가이드형 Research Brief 인터뷰를 시작합니다.");
      expect(app.logs).toContain("Adaptive guided brief mode: hypothesis_test");
      expect(app.logs).toContain("Guided brief coverage 6/15; next=researchQuestion.");
      expect(app.openSelectionMenu).toHaveBeenCalledWith(
        "연구 모드 선택",
        expect.arrayContaining([
          expect.objectContaining({
  ...
  it("keeps a model follow-up pending in the TUI instead of running a prewired action", async () => {
    const app = makeApp();
    const request = {
      id: "request-followup",
      sourceNode: "analyze_results",
    ...
    const followupRequest = {
      ...request,
      question: "Should the metric be revised, or should the run return to experiment design?",
      conversation: [{
        question: request.question,
        answer: "I am not sure.",
        followupQuestion: "Should the metric be revised, or should the run return to experiment design?",
        resolutionSource: "model",
        recordedAt: new Date().toISOString()
      }]
    };
    app.pendingHumanIntervention = { runId: "run-1", request };
    app.getNaturalAssistantClient = vi.fn().mockReturnValue({ runForText: vi.fn() });
    app.announceHumanIntervention = vi.fn();
    app.continueSupervisedRun = vi.fn();
    app.runBusyAction = async (action: (signal: AbortSignal) => Promise<void>) => {
      await action(new AbortController().signal);
    };
    app.interactiveSupervisor = {
      answerHumanIntervention: vi.fn().mockResolvedValue({
        status: "followup_required",
        request: followupRequest,
        message: followupRequest.question
      })
    };

    await app.handlePendingHumanInterventionAnswer("I am not sure.");

    expect(app.pendingHumanIntervention).toEqual({ runId: "run-1", request: followupRequest });
    expect(app.announceHumanIntervention).toHaveBeenCalledWith(followupRequest);
    expect(app.continueSupervisedRun).not.toHaveBeenCalled();
  });
