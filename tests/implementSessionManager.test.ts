...
describe("ImplementSessionManager", () => {
  ...
  it("keeps provider response details out of implementation failure projections", async () => {
    const workspace = mkdtempSync(path.join(os.tmpdir(), "autolabos-implement-safe-provider-error-"));
    tempDirs.push(workspace);
    process.chdir(workspace);
    const paths = resolveAppPaths(workspace);
    await ensureScaffold(paths);
    ...
    const runStore = new RunStore(paths);
    const run = await runStore.createRun({
      title: "Implementation Provider Failure Run",
      topic: "configured research topic",
      constraints: ["recent"],
      objectiveMetric: "primary_score"
    });
    ...
    const fixtureHome = path.join(workspace, "fixture-home");
    const authDir = path.join(fixtureHome, ".codex");
    ...
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue(fixtureHome);
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response("sensitive-provider-detail", { status: 503 }))
    );
    const eventStream = new InMemoryEventStream();
    ...
    try {
      const node = createImplementExperimentsNode({
        config: createTestConfig(),
        codex: new CodexNativeClient(workspace, { model: "configured-model" }),
        aci: new LocalAciAdapter(),
        eventStream,
        runStore,
        workspaceRoot: workspace,
      ...
      expect(projected).toContain("codex_oauth:provider_unavailable");
      expect(projected).not.toContain("sensitive-provider-detail");
    } finally {
        ...
        complete: async (prompt: string) => {
          ...
          if (prompt.includes("Target chunk: runner_body")) {
            ...
            if (runnerBodyCalls === 1) {
              throw new CodexOAuthCompletionError("provider_unavailable");
            }
            return {
              text: JSON.stringify({
  ...
  it("classifies typed Codex OAuth retry and stream-recovery dispositions", () => {
    expect(
      isTransientStagedLlmProviderError(new CodexOAuthCompletionError("provider_unavailable"))
    ).toBe(true);
    expect(
      isTransientStagedLlmProviderError(new CodexOAuthCompletionError("transport_error"))
    ).toBe(true);
    expect(
      isTransientStagedLlmProviderError(new CodexOAuthCompletionError("rate_limited"))
    ).toBe(true);
    expect(
      isProviderTerminatedStagedLlmError(new CodexOAuthCompletionError("stream_terminated"))
    ).toBe(true);
    expect(
      isTransientStagedLlmProviderError(new CodexOAuthCompletionError("stream_terminated"))
    ).toBe(false);
    expect(
      isTransientStagedLlmProviderError(new CodexOAuthCompletionError("quota_exhausted"))
    ).toBe(false);
    expect(
      isTransientStagedLlmProviderError(
