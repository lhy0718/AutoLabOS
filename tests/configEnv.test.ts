...
describe("config .env overrides", () => {
  ...
  it("defaults first-run setup to Terra/medium chat and Sol/high research", async () => {
    ...
    expect(config.providers.openai.chat_model).toBe("gpt-5.6-terra");
    ...
    expect(config.providers.openai.model).toBe("gpt-5.6-sol");
    expect(config.providers.openai.reasoning_effort).toBe("high");
    expect(config.providers.openai.experiment_model).toBe("gpt-5.6-sol");
