...
describe("CodexNativeClient fake response sequence", () => {
  ...
  it("refuses to treat a configured fake response as live provider evidence", async () => {
    process.env.AUTOLABOS_FAKE_CODEX_RESPONSE = "   ";
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);

    const client = new CodexNativeClient(process.cwd());
    const result = await client.probeChatCompatibility({ model: "configured-chat-model" });

    expect(result).toEqual({ status: "fixture_active" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("detects a fake response sequence without consuming it during the live check", async () => {
    const sequence = JSON.stringify([
      { reply_lines: ["first-sequence-entry"] },
      { reply_lines: ["second-sequence-entry"] }
    ]);
    process.env.AUTOLABOS_FAKE_CODEX_RESPONSE_SEQUENCE = sequence;
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const client = new CodexNativeClient(process.cwd());

    await expect(client.probeChatCompatibility({ model: "configured-chat-model" })).resolves.toEqual({
      status: "fixture_active"
    });
    expect(process.env.AUTOLABOS_FAKE_CODEX_RESPONSE_SEQUENCE).toBe(sequence);
    await expect(client.runForText({
      prompt: "fixture probe",
      sandboxMode: "read-only",
      approvalPolicy: "never"
    })).resolves.toContain("first-sequence-entry");
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("preserves typed OAuth failures without exposing provider details through native events", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "autolabos-codex-native-error-"));
    tempDirs.push(root);
    ...
    const client = new CodexNativeClient(process.cwd(), { model: "configured-model" });

    let thrown: unknown;
    try {
      await client.runTurnStream({
    ...
    expect(`${String(thrown)}\n${JSON.stringify(thrown)}\n${JSON.stringify(events)}`).not.toContain(
