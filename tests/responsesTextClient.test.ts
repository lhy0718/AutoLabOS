...
const ORIGINAL_FAKE_RESPONSE = process.env.AUTORESEARCH_FAKE_OPENAI_RESPONSE;
const ORIGINAL_FAKE_SEQUENCE = process.env.AUTORESEARCH_FAKE_OPENAI_RESPONSE_SEQUENCE;

afterEach(() => {
  if (ORIGINAL_FAKE_RESPONSE === undefined) {
    delete process.env.AUTORESEARCH_FAKE_OPENAI_RESPONSE;
  } else {
    process.env.AUTORESEARCH_FAKE_OPENAI_RESPONSE = ORIGINAL_FAKE_RESPONSE;
  }

  if (ORIGINAL_FAKE_SEQUENCE === undefined) {
    delete process.env.AUTORESEARCH_FAKE_OPENAI_RESPONSE_SEQUENCE;
  } else {
    process.env.AUTORESEARCH_FAKE_OPENAI_RESPONSE_SEQUENCE = ORIGINAL_FAKE_SEQUENCE;
  }
...
describe("OpenAiResponsesTextClient", () => {
  it("supports fake response sequences for offline tests", async () => {
    process.env.AUTORESEARCH_FAKE_OPENAI_RESPONSE_SEQUENCE = JSON.stringify(["first", "second"]);
