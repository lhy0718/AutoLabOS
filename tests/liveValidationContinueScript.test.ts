...
describe("live-validation continue helper", () => {
  ...
  it("allows only the default or explicit live-provider command in doctor validation", async () => {
    ...
    try {
      await execFileAsync("python3", [script], {
        env: {
          ...process.env,
          AUTOLABOS_VALIDATION_DOCTOR_COMMAND: "/doctor --unknown",
        },
      });
    } catch (error) {
    ...
    expect(failure).toContain(
      "AUTOLABOS_VALIDATION_DOCTOR_COMMAND must be /doctor or /doctor --live-provider"
    );
    expect(failure).not.toContain("validation workspace is not prepared");
  });
