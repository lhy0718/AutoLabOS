...
import {
  ...
  createAutoLabOSWebRequestListenerForTesting,
  isLoopbackWebHost,
...
describe("AutoLabOS web authentication boundary", () => {
  ...
  it("places static UI, APIs, raw artifacts, SSE, and actions behind one boundary", async () => {
    ...
    const surfaces = [
      ...
      { path: "/api/doctor/provider-probe", method: "POST" },
  ...
  it("keeps the narrow request-listener test seam behind the resolved auth boundary", async () => {
    const previousSecret = process.env[WEB_AUTH_SECRET_ENV];
    process.env[WEB_AUTH_SECRET_ENV] = fixtureSecret();
    let listener: RequestListener;
    try {
      listener = createAutoLabOSWebRequestListenerForTesting({ cwd: process.cwd() });
    } finally {
      if (previousSecret === undefined) {
        delete process.env[WEB_AUTH_SECRET_ENV];
      } else {
        process.env[WEB_AUTH_SECRET_ENV] = previousSecret;
      }
    }
    const port = await startServer(listener);

    expect(await sendRequest(port, "/api/bootstrap")).toEqual({
      status: 401,
      headers: expect.objectContaining({
        "cache-control": "no-store",
        "www-authenticate": 'Basic realm="AutoLabOS", charset="UTF-8"'
      }),
  ...
  it("documents the remote browser sign-in boundary in CLI help", () => {
    ...
    expect(result.stdout).toContain(WEB_TRUSTED_ORIGIN_ENV);
