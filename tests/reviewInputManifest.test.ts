...
describe("review input manifest", () => {
  ...
  it("accepts a pre-projection manifest without the optional venue input", async () => {
    const fixture = await seedReviewAssuranceFixture();
    ...
    manifest.inputs = manifest.inputs.filter(
      (entry) => entry.path !== "analysis/venue_viability_report.json"
    );
    manifest.schema_version = 1;
    const { content_sha256: _contentSha256, ...payload } = manifest;
    await fs.writeFile(
      manifestPath,
      JSON.stringify({ ...payload, content_sha256: hashCanonical(payload) }),
      "utf8"
    );
    ...
    expect(validation).toEqual({ valid: true, reason_codes: [] });
  });

  it("rejects a current manifest that omits the venue input inventory entry", async () => {
    const fixture = await seedReviewAssuranceFixture();
    const manifestPath = path.join(
      fixture.runDir,
      REVIEW_INPUT_MANIFEST_RELATIVE_PATH
    ...
    expect(validation.valid).toBe(false);
    expect(validation.reason_codes).toContain(
      "review_input_manifest_path_missing:analysis/venue_viability_report.json"
    );
  });
